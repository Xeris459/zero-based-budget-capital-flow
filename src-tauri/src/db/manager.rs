...
fn seed_defaults(conn: &rusqlite::Connection) -> Result<(), rusqlite::Error> {
    ...
    for (id, name, code, color) in default_banks.iter() {
        conn.execute("INSERT OR IGNORE INTO banks (id, name, code, color) VALUES (?1, ?2, ?3, ?4);", [id, name, code, color])?;
    }
    ...
    for (id, name, acc_type, bank_id, start_bal, bal) in default_accounts.iter() {
        conn.execute("INSERT OR IGNORE INTO accounts (id, name, type, bank_id, starting_balance, balance) VALUES (?1, ?2, ?3, ?4, ?5, ?6);", 
    ...
    for (id, name, parent_id) in default_categories.iter() {
        conn.execute("INSERT OR IGNORE INTO categories (id, name, parent_id) VALUES (?1, ?2, ?3);", [id, name, parent_id])?;
    }
    ...
    for (month, cat_id, planned) in default_budgets.iter() {
        conn.execute("INSERT OR IGNORE INTO budgets (month, category_id, planned) VALUES (?1, ?2, ?3);", 
